...
#ifndef ROGUE_LIKE_ANIMATIONCOMPONENT_H
#define ROGUE_LIKE_ANIMATIONCOMPONENT_H
...
private:
    ...
    public:
        ...
        const bool& play(const float& dt, float modifier_percent){
            //Update timer
            if (modifier_percent < 0.5f)
                modifier_percent = 0.5f;

            this->done = false;
            this->timer += modifier_percent * 100.f *dt;
            if(this->timer >= this->animationTimer)
            {
                //reset timer
                this->timer = 0.f;
...
#endif //ROGUE_LIKE_ANIMATIONCOMPONENT_H
